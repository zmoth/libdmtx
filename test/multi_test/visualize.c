...
void TimingCallback(Timing *timing0, Timing *timing1, int id)
{
    ...
    if (gState.displayTiming == DmtxTrue) {
        ...
        for (i = -64; i <= 64; i++) {
            DrawLine(gState.screen, 64, CTRL_COL3_X, CTRL_ROW5_Y, timing0->phi, timing0->shift + (timing0->period * i),
                     2, 0xff0000ff);
            DrawLine(gState.screen, 64, CTRL_COL3_X, CTRL_ROW5_Y, timing1->phi, timing1->shift + (timing1->period * i),
                     2, 0xff0000ff);
        }
    }
}
...
void GridCallback(AlignmentGrid *grid, int id)
{
    switch (id) {
        case 0:
            DrawSymbolPreview(gState.screen, gState.imgFull, grid, &gState, CTRL_ROW5_Y, CTRL_COL1_X + 1);
            break;
        case 1:
            DrawNormalizedRegion(gState.screen, gState.imgFull, grid, &gState, CTRL_ROW5_Y, CTRL_COL3_X);
            break;
    }
}
...
void BlitSobelGrid(SDL_Surface *screen, DmtxValueGrid *cache, int x, int y, int screenY, int screenX)
{
    ...
    surface = SDL_CreateRGBSurfaceFrom(pixbuf, width, height, 24, width * 3, rmask, gmask, bmask, 0);
 ...
 *
 */
void DrawNormalizedRegion(SDL_Surface *screen, DmtxImage *img, AlignmentGrid *region, AppState *state, int screenY,
                          int screenX)
{
    ...
    for (yImage = 0; yImage < extent; yImage++) {
        for (x = 0; x < extent; x++) {
            ...
            pFit.X = ((x - extent / 2) * (double)modulesToDisplay) / (region->colCount * extent) + pCtr.X;

            pFit.Y = ((yDmtx - extent / 2) * (double)modulesToDisplay) / (region->rowCount * extent) + pCtr.Y;
            ...
            if (xRaw < 0 || xRaw >= img->width || yRaw < 0 || yRaw >= img->height) {
                ...
                if (pRawActive.X < 0.0 || pRawActive.X >= state->activeExtent - 1 || pRawActive.Y < 0.0 ||
                    pRawActive.Y >= state->activeExtent - 1) {
                    ptrFit[0] = ptrRaw[0] / 2;
                    ptrFit[1] = ptrRaw[1] / 2;
                    ptrFit[2] = ptrRaw[2] / 2;
                } else {
    ...
    for (yImage = 0; yImage < extent; yImage++) {
        ...
        for (x = 0; x < extent; x++) {
            if ((yDmtx + shiftY) % dispModExtent != 0 && (x + shiftX) % dispModExtent != 0)
                continue;
    ...
    surface = SDL_CreateRGBSurfaceFrom(pixbuf, extent, extent, 24, extent * 3, rmask, gmask, bmask, 0);
 ...
 *
 */
void DrawSymbolPreview(SDL_Surface *screen, DmtxImage *img, AlignmentGrid *grid, AppState *state, int screenY,
                       int screenX)
{
 ...
 *
 */
void DrawPerimeterPatterns(SDL_Surface *screen, GridRegion *region, AppState *state, DmtxDirection side,
                           DmtxBarType type)
{
    DmtxVector2 pTmp, pCtr;
    DmtxVector2 gridTest;
 ...
 *
 */
void DrawPerimeterSide(SDL_Surface *screen, int x00, int y00, int x11, int y11, int dispModExtent, DmtxDirection side,
                       DmtxBarType type)
{
    Sint16 xBeg, yBeg;
    Sint16 xEnd, yEnd;
