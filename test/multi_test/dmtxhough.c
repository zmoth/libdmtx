 ...
 *
 */
int GetMaximaWeight(DmtxHoughLocal *line, int phi, int d)
{
 ...
 *
 */
DmtxPassFail VanishHoughAccumulate(DmtxHoughLocal *vanish, DmtxHoughLocal *line)
{
    int i, d, phi, val;
    int dLine, phiLine;
...
ZeroCrossing GetZeroCrossing(DmtxValueGrid *accel, int iCol, int iRow)
{
    ...
    if (OPPOSITE_SIGNS(aHere, aNext)) {
        ...
        if (!(accel->type == DmtxEdgeVertical && aCol == 0) && !(accel->type == DmtxEdgeHorizontal && aRow == 0)) {
 ...
 *
 */
ZeroCrossing SetZeroCrossingFromIndex(DmtxValueGrid *accel, int iCol, int iRow, double smidge)
{
    int sCol, sRow, sIdx;
    ZeroCrossing edge;
 ...
 *
 */
DmtxPassFail HoughLocalAccumulateEdge(DmtxHoughLocal *line, int phi, ZeroCrossing edge)
{
    double d;
    int dInt;
